...
__name(createRandomId, "createRandomId");
...
__name(toNameParts, "toNameParts");
function normalizeEmail(rawEmail, fallbackLocal = "user") {
  const normalized = String(rawEmail || "").trim().toLowerCase();
  if (normalized.includes("@")) {
    return normalized;
  }
  const localPart = String(fallbackLocal || "user").trim().toLowerCase().replace(/[^a-z0-9]+/g, ".").replace(/\.{2,}/g, ".").replace(/^\.+|\.+$/g, "");
...
__name(getPayloadData, "getPayloadData");
...
__name(buildDemoAuthBody, "buildDemoAuthBody");
...
__name(buildJsonHeaders, "buildJsonHeaders");
...
__name(shouldBypassInfinityFreeChallenge, "shouldBypassInfinityFreeChallenge");
async function solveInfinityFreeChallenge(request, response) {
  ...
  try {
    ...
    const cookieHex = [...plainBytes].map((byte) => byte.toString(16).padStart(2, "0")).join("");
    if (!cookieHex) {
      return null;
    }
    ...
    return fetch(challengeUrl.toString(), {
      ...
      headers,
      redirect: "manual",
      body: shouldIncludeBody ? request.body : void 0
    });
  } catch {
    return null;
  }
}
__name(solveInfinityFreeChallenge, "solveInfinityFreeChallenge");
...
__name(parseBodyData, "parseBodyData");
...
__name(resolveApiUpstreamOrigin, "resolveApiUpstreamOrigin");
...
__name(resolveCanonicalHost, "resolveCanonicalHost");
...
__name(proxyApiRequestToUpstream, "proxyApiRequestToUpstream");
...
__name(buildDemoTagList, "buildDemoTagList");
...
__name(normalizeProviderId, "normalizeProviderId");
...
__name(buildProviderDisplayName, "buildProviderDisplayName");
...
__name(buildProviderOAuthDemoPayload, "buildProviderOAuthDemoPayload");
var demoMigrationJobs = /* @__PURE__ */ new Map();
var postedVideosByUser = /* @__PURE__ */ new Map();
var DEMO_MIGRATION_STREAM_URL = "https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4";
...
__name(savePostedVideoForRequest, "savePostedVideoForRequest");
...
__name(extractGoogleDriveFileId, "extractGoogleDriveFileId");
...
__name(buildGoogleDriveDownloadUrl, "buildGoogleDriveDownloadUrl");
...
__name(buildGoogleDriveProxyPlaybackUrl, "buildGoogleDriveProxyPlaybackUrl");
...
__name(normalizeLinkedAccountPlaybackUrl, "normalizeLinkedAccountPlaybackUrl");
...
__name(normalizeGoogleDriveVideo, "normalizeGoogleDriveVideo");
async function uploadSourceUrlToGoogleDrive({ accessToken, sourceUrl, title, description }) {
  ...
  try {
    const sourceFetchUrl = sourceDriveFileId ? `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(sourceDriveFileId)}?alt=media` : normalizedSourceUrl || sourceUrl;
...
__name(uploadSourceUrlToGoogleDrive, "uploadSourceUrlToGoogleDrive");
...
__name(uploadSourceUrlToDropbox, "uploadSourceUrlToDropbox");
...
__name(extractPathParam, "extractPathParam");
...
__name(inferFilenameFromSourceUrl, "inferFilenameFromSourceUrl");
...
__name(inferVideoMimeType, "inferVideoMimeType");
...
__name(inferProviderFromUploadRequest, "inferProviderFromUploadRequest");
...
__name(getOrCreateDemoMigrationJob, "getOrCreateDemoMigrationJob");
function buildDemoMigrationStatusPayload(job) {
  const elapsed = Math.max(0, Date.now() - Number(job?.createdAt || Date.now()));
  let stage = "queued";
  let progress = 5;
  let completed = false;
  if (elapsed >= 2e3 && elapsed < 5e3) {
    stage = "downloading";
    progress = 30;
  } else if (elapsed >= 5e3 && elapsed < 8e3) {
    stage = "uploading";
    progress = 70;
  } else if (elapsed >= 8e3 && elapsed < 11e3) {
    stage = "finalizing";
    progress = 92;
  } else if (elapsed >= 11e3) {
    stage = "finalizing";
    progress = 100;
    completed = true;
  }
...
__name(buildDemoMigrationStatusPayload, "buildDemoMigrationStatusPayload");
function isOneOfPaths(pathname, candidates) {
  const path = normalizePathname(pathname);
  return candidates.includes(path);
}
__name(isOneOfPaths, "isOneOfPaths");
var worker_default = {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && (requestPath.startsWith("/api/v1/migration/status/") || requestPath.startsWith("/migration/status/"))) {
    ...
    if (request.method === "GET" && /^\/api\/v1\/accounts\/(google|facebook|dropbox|gdrive)\/videos/i.test(requestPath)) {
      ...
      if (provider === "google" || provider === "gdrive") {
        ...
        const videos = Array.isArray(drivePayload.files) ? drivePayload.files.map(normalizeGoogleDriveVideo) : [];
        return jsonResponse({
          status: 200,
          data: {
            videos,
      ...
      if (provider === "facebook") {
        ...
        try {
          ...
          const videos = Array.isArray(fbPayload.data) ? fbPayload.data.map((v) => ({
            id: v.id || "",
      ...
      if (provider === "dropbox") {
        ...
        try {
          ...
          const videos = Array.isArray(listPayload.entries) ? listPayload.entries.filter((entry) => videoExtensions.some((ext) => entry?.name?.toLowerCase()?.includes(ext))).map((entry) => ({
            id: entry?.id || "",
...
<body>
  ...
  <script>
    ...
    // Close this popup after a delay, or let the parent window close it
    setTimeout(function() {
      try {
