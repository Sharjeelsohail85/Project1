    ...
    if (request.method === 'GET' && requestPath.startsWith('/api/v1/oauth/')) {
      ...
      if (normalized === 'dropbox') {
        ...
        if (finalClientSecret) {
          ...
          const oauthUrl = `https://www.dropbox.com/oauth2/authorize?client_id=${encodeURIComponent(finalClientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${encodeURIComponent(scope)}&token_access_type=offline&state=${encodeURIComponent(state)}`
          return jsonResponse({
            status: 200,
            data: {
              provider: 'dropbox',
