...
const MigratePostPage = memo(function MigratePostPage() {
  ...
  const [dropboxSubTab, setDropboxSubTab] = useState('token') // 'token' or 'oauth'
  const [customDropboxClientId, setCustomDropboxClientId] = useState(() => {
    try {
      return localStorage.getItem('custom_dropbox_client_id') || ''
    } catch {
      return ''
    }
  })
  ...
  const [customOneDriveTenantId, setCustomOneDriveTenantId] = useState(() => {
    try {
      return localStorage.getItem('custom_onedrive_tenant_id') || ''
    } catch {
      return ''
    }
  })
  ...
  const onConnectProvider = useCallback(async (providerId, manualToken = '') => {
    ...
    try {
      ...
      if (normalized === 'onedrive') {
        ...
        setConnectedById((prev) => ({ ...prev, [normalized]: true }))
        setConnectionInfo('Microsoft OneDrive connected for uploads.')
        return
      }
      ...
      <Container maxWidth="lg" sx={{ py: 3 }}>
        <Stack spacing={2}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1.5, mb: 1, flexWrap: 'wrap' }}>
            ...
            >
              {showOneDriveTokenInput ? 'Hide OneDrive Options' : 'OneDrive Connection Options (Custom Tenant / Personal Account Bypass)'}
            </Button>
            <Button
              variant="outlined"
              size="small"
          ...
          {showOneDriveTokenInput && (
            <Box sx={{ p: 2.5, border: '1px solid rgba(255, 255, 255, 0.1)', bgcolor: 'rgba(255, 255, 255, 0.02)', borderRadius: 2, mb: 1 }}>
              <Box sx={{ display: 'flex', gap: 1, mb: 2, borderBottom: '1px solid rgba(255,255,255,0.1)', pb: 1 }}>
                ...
                >
                  Option A: Microsoft/OneDrive Access Token
                </Button>
                <Button
                  ...
                  sx={{ textTransform: 'none', fontSize: '0.75rem' }}
                >
                  Option B: Custom Azure App Credentials (Bypass License / Tenant)
                </Button>
              </Box>
              ...
              {onedriveSubTab === 'token' ? (
                <>
                  <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>
                    Direct Microsoft Graph Access Token Connection
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 2, fontSize: '0.85rem' }}>
                    If you are experiencing licensing restrictions (like <strong>"Tenant does not have a SPO license"</strong>), you can bypass all corporate Azure AD/SPO policies instantly. Copy an active <strong>Microsoft Graph Access Token</strong> (with <code>Files.ReadWrite</code> scope) and paste it below:
                  </Typography>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (!onedriveTokenValue.trim()) {
                          setConnectionError('Please enter a valid OneDrive/Graph token.')
                          return
                        }
                        try {
                          const accounts = getConnectedAccounts()
                          ...
                          setConnectionInfo('OneDrive connected via custom access token!')
                  ...
                  <Typography variant="caption" color="text.secondary" display="block">
                    💡 <strong>Where to get a token:</strong> You can get a temporary token instantly from the official <a href="https://developer.microsoft.com/en-us/graph/graph-explorer" target="_blank" rel="noreferrer" style={{ color: '#90caf9', textDecoration: 'underline' }}>Microsoft Graph Explorer</a>. Log in with your preferred Microsoft account (Personal or Work), click <strong>Access Token</strong> tab, copy the token, and paste it here.
                  </Typography>
                </>
              ) : (
                <>
                  <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>
                    Connect using Custom Azure/OneDrive App Details
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5, fontSize: '0.85rem' }}>
                    If you are using a personal Microsoft account (outlook.com / hotmail.com), or if your work tenant lacks a SharePoint Online (SPO) license, change the <strong>Tenant ID</strong> to <code>common</code> or <code>consumers</code> below. This forces Microsoft Graph to use consumer OneDrive instead of enterprise SPO.
                  </Typography>

                  <Stack spacing={1.5} sx={{ mb: 2 }}>
                    <Box>
                      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                        Microsoft Redirect URI (Must match what is registered in your Azure Portal):
                      </Typography>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
                      ...
                      <Box sx={{ flex: 1 }}>
                        ...
                        <input
                          type="text"
                          placeholder="e.g. common, consumers, or 9e7c38c3-..."
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                      <Button
                        variant="outlined"
                        size="small"
                        onClick={() => setCustomOneDriveTenantId('common')}
                        sx={{ textTransform: 'none', fontSize: '0.75rem' }}
                      >
                        Set to "common" (For Any Account)
                      </Button>
                      <Button
                        variant="outlined"
                        size="small"
                        onClick={() => setCustomOneDriveTenantId('consumers')}
                        sx={{ textTransform: 'none', fontSize: '0.75rem' }}
                      >
                        Set to "consumers" (For Personal Accounts Only)
                      </Button>
                      <Button
                        variant="outlined"
                        size="small"
                        onClick={() => setCustomOneDriveTenantId('9e7c38c3-66a5-4f8d-bdca-a8d195af3fff')}
                        sx={{ textTransform: 'none', fontSize: '0.75rem' }}
                      >
                        Set to Corporate Tenant
                      </Button>
                    </Box>
                    ...
                    <Stack direction="row" spacing={1.5} sx={{ pt: 1 }}>
                      <Button
                        ...
                        onClick={() => {
                          if (!customOneDriveClientId.trim()) {
                            setConnectionError('Application (Client) ID is required.')
                            return
                          }
                          try {
                            ...
                            setConnectionInfo('OneDrive Custom Application settings saved successfully! Click "Connect" next to OneDrive in the list to sign in.')
                            setConnectionError('')
                            setShowOneDriveTokenInput(false)
                          } catch (err) {
                            setConnectionError('Failed to save settings: ' + err.message)
                          }
                        }}
                        sx={{ textTransform: 'none' }}
                      >
                      ...
                      {(customOneDriveClientId || customOneDriveTenantId) && (
                        <Button
                          variant="outlined"
                          color="error"
                          onClick={() => {
                            try {
                              ...
                              setCustomOneDriveTenantId('')
                              setConnectionInfo('OneDrive custom settings cleared. Reverted to default app.')
                              setConnectionError('')
                            } catch (err) {
                              setConnectionError('Failed to clear settings: ' + err.message)
                            }
                          }}
                          sx={{ textTransform: 'none' }}
                        >
          ...
          {showDropboxTokenInput && (
            <Box sx={{ p: 2.5, border: '1px solid rgba(255, 255, 255, 0.1)', bgcolor: 'rgba(255, 255, 255, 0.02)', borderRadius: 2, mb: 1 }}>
              <Box sx={{ display: 'flex', gap: 1, mb: 2, borderBottom: '1px solid rgba(255,255,255,0.1)', pb: 1 }}>
                <Button
                  variant={dropboxSubTab === 'token' ? 'contained' : 'text'}
                  size="small"
                  onClick={() => setDropboxSubTab('token')}
                  sx={{ textTransform: 'none', fontSize: '0.75rem' }}
                >
                  Option A: Personal Access Token
                </Button>
                <Button
                  variant={dropboxSubTab === 'oauth' ? 'contained' : 'text'}
                  size="small"
                  onClick={() => setDropboxSubTab('oauth')}
                  sx={{ textTransform: 'none', fontSize: '0.75rem' }}
                >
              ...
              {dropboxSubTab === 'token' ? (
                <>
                  <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>
                    Dropbox Access Token Connection
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 2, fontSize: '0.85rem' }}>
                    Standard Dropbox API apps have a strict <strong>user limit</strong> in development mode. If the normal login popup fails, you can bypass it instantly by pasting a <strong>Personal Access Token</strong> generated from your Dropbox App Console:
                  </Typography>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ mb: 1.5 }}>
                    <input
                      type="password"
                  ...
                  <Typography variant="caption" color="text.secondary" display="block">
                    💡 <strong>Instructions:</strong> Go to the <a href="https://www.dropbox.com/developers/apps" target="_blank" rel="noreferrer" style={{ color: '#90caf9', textDecoration: 'underline' }}>Dropbox App Console</a>, select/create your app, scroll to <strong>Generated access token</strong>, and click <strong>Generate</strong>. Make sure scopes like <code>files.content.read</code>, <code>files.content.write</code>, <code>sharing.read</code>, and <code>sharing.write</code> are enabled!
                  </Typography>
                </>
              ) : (
                <>
                  <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>
                    Connect using your Own Dropbox Developer App
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 2, fontSize: '0.85rem' }}>
                    Create your own Dropbox App to run the full OAuth login flow without any user limit restrictions. Enter your <strong>App Key</strong> (Client ID) and <strong>App Secret</strong> below:
                  </Typography>
                  <Stack spacing={1.5} sx={{ mb: 2 }}>
                    <Box>
                      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                        Dropbox Redirect URI (Add this in your Dropbox App Console):
                      </Typography>
                      <code style={{ display: 'block', background: 'rgba(0,0,0,0.3)', padding: '6px 10px', borderRadius: '4px', fontSize: '0.8rem', color: '#81c784', wordBreak: 'break-all' }}>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
                      <input
                        ...
                        value={customDropboxClientId}
                        onChange={(e) => setCustomDropboxClientId(e.target.value)}
                        style={{
                          flex: 1,
                          background: 'rgba(0,0,0,0.2)',
                          border: '1px solid rgba(255,255,255,0.15)',
                      ...
                      <input
                        ...
                        value={customDropboxClientSecret}
                        onChange={(e) => setCustomDropboxClientSecret(e.target.value)}
                        style={{
                          flex: 1,
                          background: 'rgba(0,0,0,0.2)',
                          border: '1px solid rgba(255,255,255,0.15)',
                    ...
                    <Stack direction="row" spacing={1.5}>
                      <Button
                        ...
                        color="primary"
                        onClick={() => {
                          if (!customDropboxClientId.trim() || !customDropboxClientSecret.trim()) {
                            setConnectionError('Please enter both App Key and App Secret.')
                            return
                          }
                          try {
                            localStorage.setItem('custom_dropbox_client_id', customDropboxClientId.trim())
                            localStorage.setItem('custom_dropbox_client_secret', customDropboxClientSecret.trim())
                            setConnectionInfo('Custom Dropbox App credentials saved! Now click the "Connect" button next to Dropbox to authenticate with your own app.')
                            setConnectionError('')
                            setShowDropboxTokenInput(false)
                          } catch (err) {
                      ...
                      {(customDropboxClientId || customDropboxClientSecret) && (
                        <Button
                          variant="outlined"
                          color="error"
                          onClick={() => {
                            try {
                              localStorage.removeItem('custom_dropbox_client_id')
                              localStorage.removeItem('custom_dropbox_client_secret')
                              setCustomDropboxClientId('')
                              setCustomDropboxClientSecret('')
                              setConnectionInfo('Custom Dropbox credentials cleared. Reverted to default app.')
                              setConnectionError('')
                            } catch (err) {
