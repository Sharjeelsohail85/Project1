...
function extractGoogleDriveId(rawUrl) {
  ...
  if (fileIndex >= 0) {
    const dAfterFile = segments.findIndex(
      (segment, idx) => idx > fileIndex && segment === "d",
    );
    if (dAfterFile >= 0 && segments[dAfterFile + 1]) {
      return segments[dAfterFile + 1];
    }
  }
...
export function resolvePlaybackSource({ sourceType, sourceUrl, title } = {}) {
  ...
  if (
    /^\/?api\/v1\/google-drive\/stream\//i.test(url) ||
    /\/api\/v1\/google-drive\/stream\//i.test(url)
  ) {
    return {
      mode: "html5",
  ...
  if (
    /^\/?api\/v1\/video\/migration\/stream\//i.test(url) ||
    /\/api\/v1\/video\/migration\/stream\//i.test(url)
  ) {
    return {
      mode: "html5",
  ...
  if (driveId || type === "uploadgoogle") {
    // Google Drive videos must use iframe embed mode for reliable playback
    if (driveId) {
      return {
        mode: "iframe",
    ...
    // Fallback: if we have a Google Drive URL but couldn't extract ID, still use iframe
    return {
      mode: "iframe",
      src: url,
      title: toStringSafe(title),
