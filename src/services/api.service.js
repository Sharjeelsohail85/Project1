...
export const authAPI = {
  ...
  async oauthCallback(provider, code, state, redirectUri) {
    ...
    if (String(provider).toLowerCase() === 'dropbox') {
      try {
        customClientId = localStorage.getItem('custom_dropbox_client_id') || ''
        customClientSecret = localStorage.getItem('custom_dropbox_client_secret') || ''
      } catch {
        // ignore
      }
    }
    ...
    const response = await apiRequest(API_CONFIG.endpoints.auth.oauthCallback, {
      ...
      body: JSON.stringify({
        data: {
          ...
          custom_client_secret: customClientSecret.trim() || undefined,
        },
      }),
    })
