...
export function getConnectedAccounts() {
  ...
  // 1. Sync currently logged-in user if they registered/logged in with any provider (google, dropbox, facebook, etc.)
  try {
    const rawUser = storage.getItem('user_info')
    if (rawUser) {
      const mainUser = JSON.parse(rawUser)
      const provider = mainUser?.registration_type || storage.getItem('auth_provider') || 'google'
      if (provider) {
        const providerKey = provider === 'google.com' ? 'google' : provider
        const rawAccounts = storage.getItem(CONNECTED_ACCOUNTS_KEY)
        let accountsList = rawAccounts ? JSON.parse(rawAccounts) : []
        const hasMainAccount = accountsList.some(
          (a) => a.provider === providerKey && a.connected
        )
        if (!hasMainAccount) {
          accountsList.push({
    ...
    console.error('Failed to sync main user to connected accounts:', err)
  }
