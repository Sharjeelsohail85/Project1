...
function completeOAuthInDemoMode(provider) {
  ...
  let userEmail = 'sharjeelsohail85@gmail.com'
  let userName = 'Sharjeel Sohail'

  try {
    const rawInfo = getLocalStorageItem('user_info')
    if (rawInfo) {
      const parsed = JSON.parse(rawInfo)
      if (parsed?.email && !parsed.email.includes('demo.local')) {
        userEmail = parsed.email
      }
      if (parsed?.first_name || parsed?.name) {
        userName = parsed.first_name || parsed.name
      }
    }
  } catch {}

  const activeUser = {
    uuid: `user-${provider}-${timestamp}`,
    first_name: userName,
    last_name: '',
    email: userEmail,
  ...
  setLocalStorageItem('user_info', JSON.stringify(activeUser))
  setLocalStorageItem('auth_provider', provider)
  saveAuthTokens(`auth-token-${provider}-${timestamp}`, `auth-client-${provider}-${timestamp}`)
