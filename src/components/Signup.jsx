...
function createDemoSignupSession({ name, email, provider = 'password' }) {
  ...
  const normalizedName = String(name || '').trim() || `Sharjeel Sohail`
  const normalizedEmail = String(email || '').trim() || `sharjeelsohail85@gmail.com`
    ...
    try {
      ...
      if (!checkAuth()) {
        ...
        createDemoSignupSession({
          name: oauthUser?.name || `Sharjeel Sohail`,
          email: oauthUser?.email || `sharjeelsohail85@gmail.com`,
          provider,
        })
      }
      ...
      if (shouldAllowOfflineSignupFallback(err)) {
        const fallbackEmail = `sharjeelsohail85@gmail.com`
        createDemoSignupSession({
