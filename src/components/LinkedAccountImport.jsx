...
const LinkedAccountImport = memo(function LinkedAccountImport({
  ...
  const handleSelectVideo = useCallback((video) => {
    onSelectVideo?.({
      sourceType: selectedProvider === 'google' ? 'uploadGoogle'
        : selectedProvider === 'facebook' ? 'uploadFacebook'
        : selectedProvider === 'dropbox' ? 'uploadDropbox'
        : 'uploadLink',
      sourceUrl: video.url,
      title: video.title,
    })
      ...
      {connectedProviders.length > 0 && (
        <div className="linked-account-section">
          ...
          {connectedProviders.map((provider) => {
            const meta = PROVIDER_META[provider] || { icon: 'link', label: provider, color: '#888' }
            return (
              <div key={provider} className="linked-account-item">
                <div className="linked-account-item-left">
                  ...
                  <span className="linked-account-badge connected">Connected</span>
                </div>
                <div className="linked-account-item-actions">
                  ...
                  >
                    {fetchingVideos && selectedProvider === provider ? 'Loading...' : 'Browse Videos'}
                  </Button>
                  <Button
      ...
      {selectedProvider && (
        <div className="linked-account-videos">
          <h4 className="linked-account-section-title">
            Videos from {PROVIDER_META[selectedProvider]?.label || selectedProvider}
          </h4>
          ...
          {videoError && !fetchingVideos && (
            <Typography
              variant="body2"
              sx={{ color: 'rgba(255,255,255,0.7)', textAlign: 'center', py: 2 }}
            >
              {videoError}
            </Typography>
          )}
                ...
                <div key={video.id} className="linked-account-video-card">
                  <div className="linked-account-video-thumb">
                    {video.thumbnail ? (
                      ...
                      <i className="material-icons" style={{ fontSize: 36, opacity: 0.5 }}>
                        play_circle_outline
                      </i>
                    )}
                  </div>
                  <div className="linked-account-video-info">
                    <span className="linked-account-video-title">{video.title}</span>
