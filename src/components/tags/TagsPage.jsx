...
export default function TagsPage({ inlineFeedback = false, embedded = false }) {
  ...
  const handlePopularClick = async (tag) => {
    if (!tag || loading.addTag || loading.initial || loading.removeTag) return
    if (tag.selected || selectedKeys.has(tag.key)) {
      const existingUserTag = userTags.find((t) => t.key === tag.key) || tag
      await actions.removeTag(existingUserTag)
    } else {
          ...
          <div className="signup-tags signup-item transparent">
            <div className="signup-item-desc">
              ...
              <span className="desc-text">
                Project1 uses tags to personalize your experience in a transparent way.
              </span>
              ...
              {popularTags.map((tag) => {
                ...
                return (
                  <button
                    ...
                    disabled={loading.addTag || loading.initial || loading.removeTag === tag.key}
                    aria-label={isSelected ? `${tag.name} selected` : `Add ${tag.name}`}
                  >
                    <span className="signup-tag-pill-label">{tag.name}</span>
                    {isSelected ? (
                      <i className="material-icons" aria-hidden="true" style={{ fontSize: '14px', marginLeft: '4px', color: 'inherit' }}>check</i>
                    ) : (
                      <i className="material-icons" aria-hidden="true" style={{ fontSize: '14px', marginLeft: '4px', opacity: 0.6 }}>add</i>
                    )}
                  </button>
                )
              })}
