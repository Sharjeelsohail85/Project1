...
import ColorMixologyExact from './personalization/ColorMixologyExact'
...
export default function SettingsPage({
  ...
  const personalizationSettingKeys = useMemo(() => ['infiniteGridExplorer', 'css2', 'mixingItUp', 'colorMixology'], [])
  const designerCategories = useMemo(
    () => [
      ...
      {
        key: 'colorMixology',
        label: 'Color Mixology',
        icon: 'local_bar',
        colors: ['#e91e63', '#00bcd4', '#ff3d00', '#8e24aa', '#00e676'],
      },
    ],
    [],
  )
    ...
    <>
      <div id="titlebar" className={`titlebar ${titlebarActive ? 'active' : ''}`}>
        ...
        <div id="accountBox" className="titlebar-box titlebar-right">
          ...
          <div 
            id="titlebarProfile" 
            className="titlebar-item titlebar-right-item titlebar-profile"
            onClick={() => navigate('/channel')}
            role="button"
            tabIndex={0}
            style={{ cursor: 'pointer' }}
            onKeyDown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') navigate('/channel')
            }}
          >
            Username
            <i className="material-icons" aria-hidden="true">
                  ...
                  {section.items.map((item, itemIndex) => {
                    ...
                    return (
                      <Fragment key={`${section.id}-${itemIndex}`}>
                        ...
                        {isPersonalizationToggle && isPersonalizationChecked && personalizationKey === 'colorMixology' && (
                          <div className="personalization-widget-row">
                            <div className="personalization-widget-title">Color Mixology (Exact)</div>
                            <ColorMixologyExact onColorChange={applyThemeFromColor} />
                          </div>
                        )}
                      </Fragment>
                    )
