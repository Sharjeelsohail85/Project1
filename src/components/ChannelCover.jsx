...
export default function ChannelCover() {
  ...
  useEffect(() => {
    if (editableRef.current && editableRef.current.innerText !== config.text) {
      editableRef.current.innerText = config.text;
    }
  }, [config.text]);
  ...
  return (
    <div id="channel-cover-studio-container" className="w-full relative mb-6 group">
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden select-text">
          <div 
            className={`logo-wrapper-to-export public-logo-wrapper relative inline-block transition-transform duration-300 ${!isInlineEditing ? 'cursor-pointer hover:scale-[1.03] hover:brightness-110 active:scale-[0.98]' : ''}`}
        ...
        {/* Hover Banner Text Edit Indicator */}
        {!isInlineEditing ? (
          <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 bg-black/75 backdrop-blur-md text-white text-[11px] font-bold tracking-wider uppercase px-4 py-1.5 rounded-full opacity-0 hover:opacity-100 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10 flex items-center gap-2 shadow-lg border border-white/10">
            <span>✏️ Click sticker to edit text inline</span>
          </div>
        ) : (
          <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 bg-[#FFE500] text-black text-[11px] font-black tracking-wider uppercase px-4 py-1.5 rounded-full z-10 flex items-center gap-2 shadow-lg border border-black/15 animate-bounce">
            <span>✍️ Editing... Press Escape or click outside to finish</span>
          </div>
        )}
